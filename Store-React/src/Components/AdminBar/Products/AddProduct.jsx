...
export default function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(t("addProduct.successMessage", "تمت إضافة المنتج بنجاح. رقم المنتج: {{id}}").replace("{{id}}", data.productId));
          ...
          <div className="bg-[#F9F6EF] rounded-2xl shadow-lg p-6 border border-[#0A2C52]/20">
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                disabled={loading}
                className={`${primaryButtonClasses} ${loading ? "opacity-80 cursor-wait" : ""}`}
              >
                {loading ? (
                  <>
                    <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    {t("addProduct.button.loading", "جاري الإضافة...")}
                  </>
                ) : (
                  <>
                    <FiPlus size={20} />
